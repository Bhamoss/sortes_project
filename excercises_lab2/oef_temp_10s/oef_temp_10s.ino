...
int temp;         // incoming serial byte
int calibration_one_point; // 2 point is better but we cannot really do this
...
int get_raw_temp(){
  ...
  return reading - 273; // they say it returns kelvin, we will see.
}

int get_temp(){
  return get_raw_temp() + calibration_one_point;
}
...
void setup() {
  ...
  // calibrate temp
  // SEE DATASHEET ATMEGA32u4 ADC!!!
  // by only declaring this here, DO NOT use other adc
  // you can switch if you want but you have to reset it everytime :)
...
this result.
DUS READ EENS OM GARBAGE WEG TE GOOIEN EERST
   */

   // wait, read, wait, read because
  /*
...
are required. The correct temperature measurement will be the second one.
  ...
  // one point kinda sucks, but its all we got
  // the setoff can be wrong by 10 degrees
  // the gain is less worse, and could be mitigated with a second point, which we dont have

  


  calibration_one_point = ROOM_TEMP - get_raw_temp();
  Serial.print(calibration_one_point); Serial.println(" ¤C offset");
  // negative when temp overshoot, positive when it undershoots
  // so add it to raw temp.
}

void loop() {

  Serial.print(get_temp()); Serial.println(" ¤C");
